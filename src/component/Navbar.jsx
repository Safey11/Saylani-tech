...
const Navbar = () => {
  ...
  const [imageUrl, setImageUrl] = useState('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTN5XaPknTWTxdBcdC3r0_9blSi_8n3rD_2Xg&s');
  ...
  const fetchImage = async (query) => {
    ...
    try {
      ...
      setImageUrl(response.data.results.length > 0 ? response.data.results[0].urls.small : 'https://via.placeholder.com/300x200?text=No+Image+Found');
    } catch (error) {
    ...
    <div className="sticky top-0 z-50">
      <header className="text-gray-400 bg-gray-900 body-font" data-aos="fade-down">
        <div className="container mx-auto flex flex-wrap p-5 items-center justify-between">
          {/* Logo Section */}
          <div className="flex title-font font-medium items-center text-white" data-aos="fade-right">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" className="w-10 h-10 text-white p-2 bg-indigo-500 rounded-full" viewBox="0 0 24 24">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"></path>
            </svg>
            <Link to={'/'}><span className="ml-3 text-xl">Saylani Tech</span></Link>
          </div>
          ...
          <nav className={`md:ml-auto md:mr-auto flex flex-wrap items-center text-base justify-center ${isMobileMenuOpen ? 'flex flex-col absolute top-16 left-0 w-full bg-gray-900 p-5 z-40' : 'hidden md:flex'}`}>
            ...
            <div className="relative" onMouseEnter={() => setDropdownOpen(true)} onMouseLeave={() => setDropdownOpen(false)}>
              ...
              {/* Dropdown Menu */}
              {isDropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-gray-800 shadow-lg z-50 p-4 grid grid-cols-4 gap-4">
                  ...
                  <div className="col-span-3 grid grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <Link to="/softwareengineer" onClick={() => handleServiceClick('seo optimization')} className="block px-3 py-2 text-gray-300 hover:text-indigo-500">Software Engineer</Link>
                      ...
                      <Link to="/qaengineer" onClick={() => handleServiceClick('site audit')} className="block px-3 py-2 text-gray-300 hover:text-indigo-500">QA Engineer</Link>
                    </div>
                  </div>
                </div>
              )}
            ...
            <Link to="/career" className={`${location.pathname === '/career' ? 'text-white bg-indigo-500' : ''} mr-5 hover:text-white hover:bg-indigo-500 px-3 py-2 rounded transition-all duration-300`} data-aos="fade-up" data-aos-delay="500">Career</Link>
            <Link to='/contact'>
