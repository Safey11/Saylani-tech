...
const Tab = ({ children, tab, handleSetSelected, selected }) => {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1 rounded-full py-1.5 w-full justify-center lg:px-3 text-md transition-colors text-white ${
        transparent ? "lg:text-white" : "lg:text-black"
      } ${selected === tab ? "" : ""}`}
      data-aos="fade-up"
      data-aos-delay="200"
    >
...
const Content = ({ selected, dir }) => {
  return (
    <motion.div
      ...
      className={`absolute ${
        selected === 1 &&
        "lg:-left-[223px] top-[calc(100%_+_-200px)] sm:top-[calc(100%_+_-70px)]"
      } ${selected === 2 && "lg:-right-[180px] top-[calc(100%_+_-30px)]"} ${
        selected === 3 && "lg:left-36 top-[calc(100%_+_0px)]"
      } lg:top-[calc(100%_+_24px)] rounded-lg border border-neutral-200 shadow bg-gradient-to-b from-white via-white to-white p-3`}
    >
      <Bridge />
              ...
              webDevelopment?.map((item, index) => {
                return (
                  <Link
                    to={item.path}
                    key={index}
                    className="flex justify-between text-left items-center my-2 text-[13px] md:text-sm text-neutral-700 hover:text-purpleColor/90 hover:bg-gray-100 p-2 transition rounded hover:text-green"
                  >
              ...
              graphicDesigning?.map((item, index) => {
                return (
                  <Link
                    to={item.path}
                    key={index}
                    className="flex justify-between items-center text-left my-2 text-[13px] md:text-sm text-neutral-700 hover:text-purpleColor/90 hover:bg-gray-100 p-2 transition rounded hover:text-green"
                  >
                    {item.name} <FiArrowRight className="hidden lg:block" />
                  </Link>
                );
